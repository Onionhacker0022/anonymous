(function() {
    ...
    if (!isBuilder) {
        ...
        ready(function() {
            ...
            if (isMobile()) {
                // debugger
                // adds an image for all blocks with background video
                const videos = this.querySelectorAll('section[data-bg-video]');
                [].forEach.call(videos, (video) => {
                    //  || !!video.querySelector('.mbr-background-video-preview')
                    if (!video.querySelector('[video-to-image-on-mobile]')) return
                    const attr = video.querySelector('[video-to-image-on-mobile]').getAttribute('video-to-image-on-mobile')
                    console.log(video.firstChild)
                    if (!attr) return
                    const mbrbackgroundVideoPreview = document.createElement('div')
                    mbrbackgroundVideoPreview.classList.add('mbr-background-video-on-mobile');
                    mbrbackgroundVideoPreview.style.position = 'absolute';
                    mbrbackgroundVideoPreview.style.top = '0';
                    mbrbackgroundVideoPreview.style.bottom = '0';
                    mbrbackgroundVideoPreview.style.left = '0';
                    mbrbackgroundVideoPreview.style.right = '0';
                    mbrbackgroundVideoPreview.style.backgroundSize = 'cover';
                    mbrbackgroundVideoPreview.style.backgroundPosition = 'center center';
                    mbrbackgroundVideoPreview.style.backgroundImage = `url(${attr})`
                    // console.log(mbrbackgroundVideoPreview)
                    // video.insertBefore(mbrbackgroundVideoPreview, video.nextSibling)
                    video.insertBefore(mbrbackgroundVideoPreview, video.firstChild)
                });
                ...
                function elCarouselItem(element) {
                    ...
                    if (parentEl.querySelectorAll('.carousel-item.active .hidden.animate__animated').length) {
                        ...
                        if (element.parents().some(function(el) {
        ...
        $(document).on('add.cards', function(event) {
            ...
            if (eventTarget.classList.contains('mbr-slider') || eventTarget.classList.contains('mbr-gallery')) {
                ...
                if (eventTarget.classList.contains('mbr-slider')) {
                    eventTarget.querySelectorAll('.full-screen .slider-fullscreen-image').forEach(function(x) {
                        x.style.height = '1px'
                    });
                }
            }
    ...
    ready(function() {
        if (!isBuilder) {
            ...
            var modal = function(item) {
                ...
                item.parents('section').forEach(function(el) {
    ...
    function initClientCarousel(card) {
        ...
        target.querySelectorAll('.carousel-inner').forEach(function(el) {
    ...
    function setGradient(section) {
        ...
        if (gradientElement) {
            ...
            if (!Array.from(section.querySelectorAll('svg')).some(function(el) {
    ...
    function setProgressElement(el, cardID, percent) {
        ...
        el.querySelectorAll('.svg-gradient > *').forEach(function(el) {
        ...
        $(document).on('add.cards changeParameter.cards', function(event, paramName) {
            ...
            if (paramName) {
                ...
                if (event.target.querySelectorAll('.pie_progress').length) {
                    event.target.querySelectorAll('.pie_progress').forEach(function(el) {
